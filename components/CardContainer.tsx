...
const CardContainer: React.FC<CardContainerProps> = ({ card, onReveal }) => {
  ...
  return (
    <div className="relative w-full max-w-sm mx-auto  bg-yellow-500 rounded-xl shadow-2xl overflow-hidden border-8 border-yellow-600 p-4 select-none">
      {/* Background Pattern */}
      <div
      ...
      <div className="relative z-0 text-center mb-4">
        <h2 className="text-3xl font-black text-red-700 italic drop-shadow-md">
          {card.title}
        </h2>
      ...
      <div className="grid grid-cols-1 gap-4 h-full">
        ...
        <div className="bg-red-100 rounded-lg p-2 border-2 border-red-300">
          <p className="text-[10px] font-bold text-red-800 text-center mb-1">
            中獎號碼
          </p>
          <div className="flex justify-center gap-4">
            {card.winningNumbers.map((num, i) => (
              <div
                key={i}
                className="w-10 h-10 bg-yellow-400 rounded-full flex items-center justify-center text-xl font-black text-red-800 border-2 border-red-400 shadow-inner"
              >
                {num}
              </div>
            ))}
        ...
        >
          {card.yourNumbers.map((item, i) => (
            <div
              key={i}
              className={`flex flex-col items-center justify-center p-1 rounded border ${
                item.isWinner && revealed
                  ? "bg-yellow-200 border-yellow-500 scale-105 transition-transform"
                  : "border-gray-200"
              }`}
            >
              <span
                className={`text-2xl font-black ${
                  item.isWinner && revealed ? "text-red-600" : "text-gray-800"
                }`}
              >
                {item.number}
              </span>
